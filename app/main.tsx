...
import { Grid, GridColumn as Column, GridDataStateChangeEvent, GridExpandChangeEvent, GridToolbar } from '@progress/kendo-react-grid';
...
const App = () => {
    ...
    const onGroupsToggle = React.useCallback(
        () => {
            const dataStateWithoutPaging = processWithGroups(products, {group: dataState.group});

            setCollapsedState(collapsedState.length ? [] : getGroupIds({data: dataStateWithoutPaging.data}));
        },
